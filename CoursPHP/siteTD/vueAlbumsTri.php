...
if ($_GET["tri"] == 'genre') {
    $sql = "SELECT * FROM album A inner join genre G on A.num=G.num order by libelle";// on écrit la requête sous forme de chaine de caractères
} else if ($_GET["tri"] == 'dessinateur') {
    $sql = "SELECT * FROM album A inner join genre G on A.num=G.num order by dessinateur";
}
$i = 0;
try {
